...
// Replaces the im2col+GEMM conv with cuDNN (fwd + bwd data/filter/bias); grouped/depthwise
// convs (yolo11 depthwise head) use cudnnSetConvolutionGroupCount.
...
inline DT dconv2d(DT in, DT w, DT bias, int64_t stride, int64_t pad, int64_t groups = 1) {
  ...
  int64_t Cog = Cout/groups, Kg = Cig*kh*kw, P = OH*OW;
  DT o = dmake({N, Cout, OH, OW});
#if defined(USE_CUDA) && defined(USE_CUDNN)
  // ---- cuDNN backend (grouped): filter Cin dim = Cig (=Cin/groups); group count = groups.
  // CROSS_CORRELATION matches im2col; beta=1 on bwd accumulates into grad (like +=).
  {
    ...
    CUDNN_CHECK(cudnnSetFilter4dDescriptor(fd, CUDNN_DATA_FLOAT, CUDNN_TENSOR_NCHW, (int)Cout,(int)Cig,(int)kh,(int)kw));
    ...
    CUDNN_CHECK(cudnnSetConvolutionGroupCount(cvd, (int)groups));
  ...
  o->backward_fn = [in,w,bias,N,Cin,H,Wd,Cout,Cig,kh,kw,OH,OW,stride,pad,groups, oo=o.get()]() {
    cudnnHandle_t h = bk::bk_cudnn();
    cudnnTensorDescriptor_t xd, yd; cudnnFilterDescriptor_t fd; cudnnConvolutionDescriptor_t cvd;
    ...
    cudnnSetFilter4dDescriptor(fd, CUDNN_DATA_FLOAT, CUDNN_TENSOR_NCHW, (int)Cout,(int)Cig,(int)kh,(int)kw);
    cudnnSetConvolution2dDescriptor(cvd, (int)pad,(int)pad,(int)stride,(int)stride,1,1, CUDNN_CROSS_CORRELATION, CUDNN_DATA_FLOAT);
    cudnnSetConvolutionGroupCount(cvd, (int)groups);
    float one=1.f;
    ...
    { cudnnConvolutionBwdFilterAlgoPerf_t pf; int got=0;
    ...
    { cudnnConvolutionBwdDataAlgoPerf_t pd; int got=0;
...
// ---- ops for attention (C2PSA): reshape / transpose2d / softmax_rows / mul_scalar / batch slice+concat ----
struct MulCf { float c; DHD float operator()(float v) const { return v * c; } };
inline DT dreshape(DT a, std::vector<int64_t> shape) {         // same numel, relabel
  DT y = dmake(std::move(shape)); thrust::copy(a->data.begin(), a->data.end(), y->data.begin());
  y->parents = {a};
  y->backward_fn = [a, yo=y.get()]() { thrust::transform(a->grad.begin(), a->grad.end(), yo->grad.begin(), a->grad.begin(), thrust::plus<float>()); };
  return y;
}
inline DT dtranspose2d(DT A) {                                 // [M,N] -> [N,M]
  int64_t M = A->shape[0], N = A->shape[1]; DT y = dmake({N, M});
  const float* a = A->dp(); float* o = y->dp();
  bk::parallel_for(M*N, [=] BK_HD (int64_t idx) { int64_t m=idx/N, n=idx%N; o[n*M+m] = a[idx]; });
  y->parents = {A};
  y->backward_fn = [A, M, N, yo=y.get()]() { const float* g = yo->gp(); float* ga = A->gp();
    bk::parallel_for(M*N, [=] BK_HD (int64_t idx) { int64_t m=idx/N, n=idx%N; ga[idx] += g[n*M+m]; }); };
  return y;
}
inline DT dmul_scalar(DT a, float c) {
  DT y = dmake(a->shape); thrust::transform(a->data.begin(), a->data.end(), y->data.begin(), MulCf{c});
  y->parents = {a};
  y->backward_fn = [a, c, yo=y.get()]() { thrust::device_vector<float> t(a->numel());
    thrust::transform(yo->grad.begin(), yo->grad.end(), t.begin(), MulCf{c});
    thrust::transform(a->grad.begin(), a->grad.end(), t.begin(), a->grad.begin(), thrust::plus<float>()); };
  return y;
}
inline DT dsoftmax_rows(DT a) {                                // softmax over the last dim
  int64_t C = a->shape.back(), R = a->numel()/C; DT y = dmake(a->shape);
  const float* x = a->dp(); float* o = y->dp();
  bk::parallel_for(R, [=] BK_HD (int64_t r) { const float* xr = x+r*C; float* orr = o+r*C;
    float m = -3.4e38f; for (int64_t c=0;c<C;++c) if (xr[c]>m) m=xr[c];
    float s = 0; for (int64_t c=0;c<C;++c){ float e = expf(xr[c]-m); orr[c]=e; s+=e; }
    for (int64_t c=0;c<C;++c) orr[c] /= s; });
  y->parents = {a};
  y->backward_fn = [a, C, R, yo=y.get()]() { const float* yv = yo->dp(); const float* g = yo->gp(); float* ga = a->gp();
    bk::parallel_for(R, [=] BK_HD (int64_t r) { const float* yr=yv+r*C; const float* gr=g+r*C; float* gar=ga+r*C;
      float dot=0; for (int64_t c=0;c<C;++c) dot += yr[c]*gr[c];
      for (int64_t c=0;c<C;++c) gar[c] += yr[c]*(gr[c]-dot); }); };
  return y;
}
inline DT dslice_batch(DT x, int64_t b) {                      // x[b] -> [1,C,H,W]
  int64_t C=x->shape[1],H=x->shape[2],W=x->shape[3],CHW=C*H*W; DT y=dmake({1,C,H,W});
  thrust::copy(x->data.begin()+b*CHW, x->data.begin()+(b+1)*CHW, y->data.begin());
  y->parents={x};
  y->backward_fn=[x,b,CHW, yo=y.get()](){ thrust::transform(yo->grad.begin(), yo->grad.end(), x->grad.begin()+b*CHW, x->grad.begin()+b*CHW, thrust::plus<float>()); };
  return y;
}
inline DT dconcat_batch(std::vector<DT> xs) {                  // stack along batch
  int64_t B=(int64_t)xs.size(),C=xs[0]->shape[1],H=xs[0]->shape[2],W=xs[0]->shape[3],CHW=C*H*W; DT y=dmake({B,C,H,W});
  for (int64_t b=0;b<B;++b) thrust::copy(xs[b]->data.begin(), xs[b]->data.end(), y->data.begin()+b*CHW);
  y->parents=xs;
  y->backward_fn=[xs,B,CHW, yo=y.get()](){ for (int64_t b=0;b<B;++b) thrust::transform(yo->grad.begin()+b*CHW, yo->grad.begin()+(b+1)*CHW, xs[b]->grad.begin(), xs[b]->grad.begin(), thrust::plus<float>()); };
  return y;
}
